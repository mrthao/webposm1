    ...
    function (
        ...
        return Component.extend({
            ...
            showPopup: function (type) {
                var viewManager = require('view/layout');
                this.isShowActionPopup(false);
                if (!this.popupArray) {
                    this.popupArray = {
            ...
            checkValue: function (data) {
                if(data.totalLabel == 'Gift Card'){
                    data.totalValue = priceHelper.formatPrice(-priceHelper.toNumber(data.totalValue));
                }

                if (priceHelper.toNumber(data.totalValue) == 0 && data.totalLabel == 'Change') {
                    return false;
                } else {
                    return true;
                }
            ...
            getWebposPaymentAmount: function (data) {
                ...
                return (data.base_payment_amount == 0) ? this.convertAndFormatPrice(0) : formatedAmount;
            },
                    ...
                    if(this.showIntegration()){
                        ...
                        if(hasRewardpoints){
                            ...
                            payments.push({
                                base_payment_amount:-baseAmount,
                                payment_amount:-amount,
                                method_title: ("Customer's Reward Points")
                            });
                        }
                    }
                }
