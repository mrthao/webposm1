...
define(
    ...
    function ($, ko, Items, Totals, Event, DataManager, CartResource, TaxCalculator, poleHelper, generalHelper, ProductFactory) {
        "use strict";
        var CartModel = {
                        ...
                        cdeferred.done(function (data) {
                            if (data.items[0]) {
                                ...
                                if(child && child.tier_prices){
                                    ...
                                    $.each(child.tier_prices, function(index, data) {
                                        if(
                                            ((self.customerGroup() == data.cust_group) || (data.all_groups == '1'))
                                            && (itemQty >= parseFloat(data.price_qty))
                                        ){
                    ...
                    ko.utils.arrayForEach(hasNoGroupPriceItems, function(item) {
                        ...
                        cdeferred.done(function (data) {
                            if (data.items[0]) {
                                var child = data.items[0];
                                var group_price = false;
                                if(child && child.group_price){
                                    item.group_prices(child.group_price);
                                    var validGroupPrice = ko.utils.arrayFirst(child.group_price, function(data) {
