...
class Magestore_Webpos_Model_Cart_Data_ItemRequest extends Magestore_Webpos_Model_Abstract implements Magestore_Webpos_Api_Cart_ItemRequestInterface
{
    ...
    public function convertData(){
        ...
        if(!empty($buyRequest['super_attribute'])){
            $buyRequest['super_attribute'] = $this->convertOption($buyRequest['super_attribute']);
        }
        if(!empty($buyRequest['options'])){
            $buyRequest['options'] = $this->convertOption($buyRequest['options']);
        }
        if(!empty($buyRequest['bundle_option_qty'])){
            $buyRequest['bundle_option_qty'] = $this->convertOption($buyRequest['bundle_option_qty']);
        }
        if(!empty($buyRequest['bundle_option'])){
            $buyRequest['bundle_option'] = $this->convertOption($buyRequest['bundle_option']);
        }
