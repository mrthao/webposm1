...
class Magestore_Webpos_Helper_Order extends Mage_Core_Helper_Abstract {
    ...
    public function getAllOrderInfo($order) {
        ...
        $paymentrObject = Mage::getModel('webpos/payment_orderPayment')->getCollection()->addFieldToFilter('order_id', $order->getId());
        $order->setData('webpos_order_payments', $paymentrObject->getData());
...
//        $order->setData('created_at', $order->getCreatedAtStoreDate()->toString("MM/dd/YYYY HH:mm:ss a"));
    ...
    public function getOrderData($order) {
        ...
        if ($invoiceIds && isset($invoiceIds[0]))
            $orderData['invoice_id'] = $invoiceIds[0];
...
//        $orderData['created_at'] = $order->getCreatedAtStoreDate()->toString("MM/dd/YYYY HH:mm:ss a");
        ...
        $orderData['webpos_order_payments'] = $paymentrObject->getData();
